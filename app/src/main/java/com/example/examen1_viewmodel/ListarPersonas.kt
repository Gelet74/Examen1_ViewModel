...
fun ListarPersona(viewModel: AppViewModel = viewModel()) {
    Column(
        ...
        LazyVerticalGrid(columns = GridCells.Fixed(2)) {
            // Mostrar alumnos
            items(viewModel.appUIState.value.alumnos) { alumno ->
                PersonaCard(
                    rol = "Alumno",
                    nombre = alumno.nombre,
                    curso = alumno.cursoSeleccionado,
                    codigo = alumno.codIden,
                    nia = alumno.NIA
                )
            }

            // Mostrar profesores
            items(viewModel.appUIState.value.profesores) { profesor ->
                PersonaCard(
                    rol = "Profesor",
                    nombre = profesor.nombre,
                    curso = profesor.cursosImparte,
                    codigo = profesor.codigoIdent,
                    esTutor = profesor.esTutor
                )
            }
        }
